# ==========================================================================
#  EKS: _eks_node-devops.tf (EKS Node Configuration)
# --------------------------------------------------------------------------
#  Description
# --------------------------------------------------------------------------
#    - EKS Node Group Name
...
#============================================
locals {
  node_selector_devops = "devopspoc"
}
...
resource "aws_eks_node_group" "devops" {
  ...
  instance_types = local.env == "prod" ? ["t3.xlarge"] : ["t3.large"]
  disk_size      = 30
  ...
  scaling_config {
    desired_size = "${each.key}" == "mon" ? 1 : 0
    max_size     = 5
    min_size     = "${each.key}" == "mon" ? 1 : 0
  }
...
output "eks_node_name_devops_monitoring" {
  value = aws_eks_node_group.devops["mon"].id
}
...
output "eks_node_tg_devops_monitoring" {
  value = aws_lb_target_group.devops["mon"].id
}
